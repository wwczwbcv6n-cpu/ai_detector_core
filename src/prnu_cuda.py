...
PRNUExtractorGPU.extract_both() returns:
    (prnu_feats (B,32), prnu_map (B,3,64,64))

Scalar features layout matches extract_prnu_features_fullres() where possible:
    ...
    [16..19] — PRNU energy in 4 freq bands via torch.fft.rfft2
    [20..23] — cross-channel correlations via torch.corrcoef
...
class PRNUExtractorGPU(nn.Module):
    ...
    def __init__(self, device: torch.device, map_size: int = 64, sigma: float = 1.5):
        super().__init__()
    ...
    def extract_features(self, img: torch.Tensor) -> torch.Tensor:
        """
        Extract 32-dim scalar PRNU feature vector (GPU path).
        ...
        feats = torch.zeros(B, 32, device=self.device, dtype=torch.float32)
    ...
    def extract_both(self, img: torch.Tensor):
        ...
        Returns:
            (prnu_feats (B,32), prnu_map (B,3,map_size,map_size))
        """
        img   = img.to(self.device)
        noise = self._extract_noise(img)                   # (B,3,H,W)
...
if __name__ == '__main__':
    ...
    print(f"  feats: {feats.shape}  map: {nmap.shape}")  # (2,32)  (2,3,64,64)
    assert feats.shape == (2, 32), f"Expected (2,32), got {feats.shape}"
    assert nmap.shape  == (2, 3, 64, 64), f"Expected (2,3,64,64), got {nmap.shape}"
